...
Engagement workspace for the **Ironbark Industrial Supply Pty Ltd** simulation, the
**Sales Cloud Consultant** certification track. Hemayet plays the consultant; the
company and every record in it are fictional. See `README.md` for the brief and scope.

## The org

Target org alias **`ironbark`**, a Developer Edition org authorised locally.

```bash
sf org display --target-org ironbark
sf data query --target-org ironbark --query "SELECT COUNT() FROM Account"
```
...
- **`.mcp.json`** starts the Salesforce DX MCP server pinned to the **`ironbark`**
  ...
  `sf org login web --alias ironbark` first.
